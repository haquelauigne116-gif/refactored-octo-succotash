...
每个会话对应一个独立文件夹:
  data/session/{sid}/
    chat.md            — 纯文本对话记录 (不含 HTML，人类可读 + AI 上下文)
    render.jsonl       — 渲染元数据 (工具卡片、媒体路径等，仅前端使用)
    ai_context.jsonl   — AI 观察日志 (工具获取的信息摘要，供 AI 推理参考)
...
class SessionManager:
    ...
    def __init__(self):
        ...
        self.ai_context_path: str = ""  # AI 观察日志
        self.render_path: str = ""      # 渲染元数据
    ...
    def create_new(self):
        ...
        self.ai_context_path = os.path.join(self.session_dir, "ai_context.jsonl")
        self.render_path = os.path.join(self.session_dir, "render.jsonl")
    ...
    def switch_to(self, session_id: str) -> dict | None:
        ...
        self.ai_context_path = os.path.join(target_dir, "ai_context.jsonl")
        self.render_path = os.path.join(target_dir, "render.jsonl")
        ...
        return {
            ...
            "render_events": self.load_render_events(),
        }
    ...
    @staticmethod
    def _strip_html_for_chat(text: str) -> str:
        """去除渲染用 HTML，只保留纯文本写入 chat.md"""
        if not text:
            return text
        # 移除工具卡片容器
        text = re.sub(r'<div class="tool-cards-row">.*?</div><!-- END_TOOLS -->', '', text, flags=re.DOTALL)
        # 移除媒体网格，替换为纯文本标记
        text = re.sub(r'<div class="media-grid">.*?</div><!-- END_MEDIA_GRID -->', '[图片已展示]', text, flags=re.DOTALL)
        # 移除视频容器
        text = re.sub(r'<div class="media-video">.*?</div>', '[视频已展示]', text, flags=re.DOTALL)
        # 清理多余空行
        text = re.sub(r'\n{3,}', '\n\n', text)
        return text.strip()

    def append_ai_message(self, text: str, thinking: str = "", tool_summary: str = ""):
        """追加 AI 回复到内存和文件（含可选的思考内容和工具调用摘要）"""
        # 内存中保留原始文本（供当前轮使用）
        ...
        # 写入 chat.md 时去掉 HTML
        clean_text = self._strip_html_for_chat(text)
        with open(self.file_path, "a", encoding="utf-8") as f:
            if thinking:
                f.write(f"[思考]{thinking}[/思考]\n")
            if tool_summary:
                f.write(f"[工具调用] {tool_summary}\n")
            f.write(f"小鱼: {clean_text}\n\n")

    def append_render_event(self, event: dict):
        """追加渲染事件到 render.jsonl（工具卡片、媒体路径等 UI 元数据）"""
        try:
            with open(self.render_path, "a", encoding="utf-8") as f:
                f.write(json.dumps(event, ensure_ascii=False) + "\n")
        except Exception as e:
            print(f"[Session] render.jsonl 写入失败: {e}")

    def load_render_events(self) -> list[dict]:
        """加载所有渲染事件"""
        events = []
        if os.path.exists(self.render_path):
            try:
                with open(self.render_path, "r", encoding="utf-8") as f:
                    for line in f:
                        line = line.strip()
                        if line:
    ...
    # ---------- AI 上下文日志 (ai_context.jsonl) ----------

    def append_ai_context(self, entry: dict):
        """追加一条 JSON 记录到 ai_context.jsonl（Plan/Act/Observe 日志）"""
        os.makedirs(self.session_dir, exist_ok=True)
        with open(self.ai_context_path, "a", encoding="utf-8") as f:
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")

    def load_ai_context(self, last_n_turns: int = 0) -> list[dict]:
        """加载 AI 上下文日志。last_n_turns=0 表示加载全部。"""
        if not os.path.exists(self.ai_context_path):
            return []
        entries: list[dict] = []
        try:
            with open(self.ai_context_path, "r", encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
                    if line:
            ...
            pass
        if last_n_turns > 0 and entries:
            # 只保留最近 N 个 turn 的记录
            max_turn = max(e.get("turn", 0) for e in entries)
            min_turn = max(1, max_turn - last_n_turns + 1)
            entries = [e for e in entries if e.get("turn", 0) >= min_turn]
        return entries

    def get_turn_number(self) -> int:
        """获取当前对话轮次号（基于 ai_context.jsonl 中最大的 turn 值 + 1）"""
        entries = self.load_ai_context()
        if not entries:
            return 1
        return max(e.get("turn", 0) for e in entries) + 1
